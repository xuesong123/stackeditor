...
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
const ID = {"timestamp": 0};

ID.next = function() {
    let t = Math.floor(new Date().getTime() / 1000) * 1000;

    if(t > this.timestamp) {
        return (this.timestamp = t);
    }
    return (this.timestamp = this.timestamp + 1000);
};

ID.format = function() {
    const date = new Date(this.next());
 ...
 */
MenuItem.prototype.render = function(bind) {
    if(this.spec && this.spec.render) {
        return this.spec.render.apply(this, [bind]);
    }
...
MenuBar.prototype.render = function() {
    ...
    for(let i = 0; i < this.items.length; i++) {
        ...
        const element = item.render(this.bind);
        const buttons = element.querySelectorAll("button.se-button");

        if(item instanceof DropdownMenu) {
            for(let j = 0; j < buttons.length; j++) {
                buttons[j].role = "menuitem";
            }
            // 下拉菜单
...
export {DOM, Events, Bytes, ID, DataURI, FileType, FilePicker, SVG, Icon, Resource, Template, Value, Env, Draggable, Resizable, MenuBar, Divider, MenuItem, DropdownMenu, DropdownItem, ContextMenu};
