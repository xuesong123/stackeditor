...
const {DOM, Events, Bytes, ID, DataURI, FileType, FilePicker, SVG, Icon, Resource, Template, Value, Env, Draggable, Resizable, MenuBar, Divider, MenuItem, DropdownMenu, DropdownItem, ContextMenu} = StackEditorWidget;
...
Toolbar.defaults = function() {
    ...
    t.zoom = new MenuItem({
        "name": "zoom",
        "label": I18N.get("tool.zoom.label"),
        "title": I18N.get("tool.zoom.title"),
        "render": function(bind) {
            const element = document.createElement("div");
            element.className = "se-menuitem";
            element.setAttribute("role", "group");
            element.innerHTML = [
                "<button type=\"button\" class=\"se-button left\" role=\"button\" aria-label=\"缩小\" data-name=\"zoom\" data-title=\"缩小\">" + Icon.get("minus") + "</button>",
                ...
                "<button type=\"button\" class=\"se-button right\" role=\"button\" aria-label=\"放大\" data-name=\"zoom\" data-title=\"放大\">" + Icon.get("plus") + "</button>"
            ].join("");
            return (this.element = element);
        },
        "run": EditCommand.zoom
    });
...
Toolbar.getItems = function(customs, enabled) {
    // save | cut copy paste |
    const all = "format, fontfamily, fontsize, quote, |, strong, italic, underline, strike, forecolor, backcolor, clean, |, ulist, olist, lift, align, dedent, indent, |, link, unlink, anchor, image, video, audio, attachment, hr, emot, table, code, undo, redo, fullscreen, |, zoom, |, assets, share, lock, info, setting";
...
HtmlPasteHandler.process = function(view, event, slice) {
    const handle = function(html) {
        if(html) {
            const cleaned = HtmlPasteHandler.clean(html);
    ...
    // 暂不使用异步读取
    // if(Env.safe()) {
    //     HtmlPasteHandler.read(handle);
    // }
    // else {
    //     handle(event.clipboardData.getData("text/html"));
    // }
    return handle(event.clipboardData.getData("text/html"));;
};

HtmlPasteHandler.read = function(handle) {
    window.navigator.clipboard.read().then(function(items) {
        for(const item of items) {
            if(item.types.includes("text/html")) {
                item.getType("text/html").then(function(blob) {
                    blob.text().then(handle);
                });
                break;
            }
        }
...
EditorViewBuilder.build = function(element, opts) {
    ...
    // plugins.push(PaginationPlugin.create());
    ...
    const handle = function(event) {
        ...
        if(dom == null || dom == undefined || dom.parentNode == null) {
            document.removeEventListener("paste", handle);
            return;
        }

        if(dom.contains(event.target) != true) {
            return;
        }
...
StackEditor.prototype.render = function(container, value) {
    ...
    const element = document.createElement("div");
    element.className = "se-container";
    element.innerHTML = [
...
StackEditor.prototype.getValue = function(format) {
...
StackEditor.prototype.setValue = function(content, prepare) {
    let element = null;

    if(content instanceof window.HTMLElement) {
        if(prepare == true) {
            element = document.createElement("div");
            element.innerHTML = HTML.clean(content.innerHTML);
        }
        else {
            element = content;
        }
        ...
        element = document.createElement("div");
        element.innerHTML = (prepare == true ? HTML.clean(content) : content);
    }
...
StackEditor.prototype.enable = function(b) {
    const container = this.view.dom.parentElement;
    const title = container.querySelector(":scope > div.se-title input[name=title]");

    if(title) {
        title.readOnly = (b == false ? true : false);
    }

    this.view.dispatch(this.view.state.tr.setMeta("readonly", (b == false ? true : false)));
...
export {StackEditor, EditCommand, DefaultUpload, Base64Upload, MockUpload, ResourceHandler, HTML, DOM, Ajax, Sharing, BundleManager};
